...
Open Bedrock Server is a unified, provider-agnostic chat completions API server. It exposes a single `/v1/chat/completions` endpoint that auto-detects request format (OpenAI, Bedrock Claude, Bedrock Titan), routes to the appropriate provider based on model ID, and converts between formats seamlessly. Built with FastAPI + Pydantic, uses boto3 for AWS Bedrock and the openai SDK for OpenAI.
...
1. Create a new strategy in `adapters/bedrock/` implementing `BedrockAdapterStrategyABC`
